...
const Avatar: React.FC<AvatarProps> = ({ avatarUrl, onAvatarClick, loading = false }) => {
  const imageNumber = avatarUrl.split('/').pop()?.split('.')[0] || '0';
      ...
      <div className="w-full">
        <div className="bg-sub-black rounded-md shadow-xl duration-300 hover:scale-105 dark:hover:bg-gray-700 hover:bg-gray-200 sm:p-3 p-2">
          <div className="w-full sm:mb-3 mb-2">
            {loading || imageError ? (
              <div className="bg-gray-200 rounded-full shadow-xl h-20 w-20 animate-pulse"></div>
            ) : (
              <img
                className="w-full shadow-md rounded-full cursor-pointer"
                src={avatarUrl}
          ...
          <div className="flex items-center justify-between gap-2">
            {loading || imageError ? (
              ...
              <a
                ...
                className="sm:h-7 h-6 p-1.5 rounded-full bg-white bg-opacity-20 hover:bg-green fill-main-white hover:fill-main-black"
